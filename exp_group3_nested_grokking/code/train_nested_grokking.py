"""
嵌套 Grokking 实验：模乘法
探索更长训练时间和更强正则化下的二次泛化现象

任务：(a * b) mod p
在超长训练（1M steps）中观察 grokking 之后是否出现更深层的结构发现

用法（在 Docker 容器内）：
    python /workspace/ai-theorys-study/arxiv/wechat67/exp_group3_nested_grokking/code/train_nested_grokking.py --wd 2.0 --seed 42
"""
...
def train(config):
    ...
    print(f"Weight decay: {config['weight_decay']}, Seed: {config['seed']}")
    ...
    while step < config["total_steps"]:
        for batch_a, batch_b, batch_y in train_loader:
            ...
            if step % config["save_activations_every"] == 0:
                ...
                # 保存
                np.savez(
                    os.path.join(config["output_dir"], "activations", f"step_{step:07d}.npz"),
                    hidden=all_hidden,
                    labels=all_labels
                )
