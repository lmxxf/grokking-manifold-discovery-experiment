...
| **激活提取** | PyTorch | 最后一层 Transformer 输出，取第一个 token 的输出，得到 $n \times 128$ 矩阵 |
...
### 6.9 实验组3：嵌套 Grokking——拓扑夺舍与容量假说

#### 6.9.1 动机

6.4 节发现模乘法模型学到了 $\mathbb{Z}_{12}$ 商群结构（12 个陪集，邻接得分 100%），但每个陪集内部的 8 个元素（$\mathbb{Z}_8$ 子结构）是死记硬背的。**能否通过更强的正则化和更长的训练，逼模型也发现 $\mathbb{Z}_8$ 的内部结构？**

#### 6.9.2 实验配置

| 参数 | 小模型（基线） | 大模型（扩容） |
|------|----------------|----------------|
| 任务 | $(a \times b) \mod 97$ | 同左 |
| 层数 | 2 | 4 |
| Hidden dim | 128 | 256 |
| Attention heads | 4 | 8 |
| 参数量 | ~434K | ~3.2M |
...
**分析工具**：`scan_strides.py`——对外层（12 个陪集）和内层（每陪集 8 个元素）分别检测 stride=1/2/4 的邻接得分。stride 是"步长"：stride=1 表示标准环（0→1→2→...→11→0），stride=2 表示隔一跳（偶数一圈、奇数一圈），stride=4 表示隔三跳。stride≠1 意味着模型选择了一种压缩版的拓扑编码。

#### 6.9.3 发现 1：拓扑夺舍

不管 WD 多少（只要能触发 Grokking），外层 $\mathbb{Z}_{12}$ 结构最终都会坍塌，被内层 stride=4 结构取代。**全程 test_acc=100%**——模型在内部表示完全重组的过程中，输出始终正确。

**wd=2.0 时间线**（关键步骤摘录）：

| Step | outer_s2 | inner_s4 | PCA dim | 解读 |
|------|----------|----------|---------|------|
| 20,000 | **1.000** | 0.115 | 13 | 外层 stride=2 锁定 |
| 100,000 | 1.000 | 0.208 | 11 | 内层开始有信号 |
| 140,000 | 0.583 | 0.000 | 6 | **转折点**，PCA 骤降 |
| 200,000 | 0.083 | 0.542 | 13 | 内层起飞 |
| 860,000 | 0.000 | **0.896** | 12 | 内层最高点 |
| 1,000,000 | 0.083 | 0.844 | 15 | 最终（仍在震荡） |

**wd=1.5 时间线**：外层选择 stride=1（标准大环），存活到 ~350K 步后崩溃，同样被内层 stride=4 接管。

**结论**：小模型的容量（~100K 参数）不足以同时维持两层拓扑结构，只能二选一——先学外层（粗分类更容易），后被内层夺舍（stride=4 编码效率更高）。

#### 6.9.4 发现 2：WD 决定拓扑选择

| WD | 外层拓扑选择 | 解读 |
|----|-------------|------|
| 1.5 | stride=1（12步标准大环） | WD 压力适中，模型负担得起精确结构 |
| 2.0 | stride=2（两个6步小环） | WD 压力大，选择有损压缩——拆成两个子环，降低权重耦合度 |

stride=2 将 $\mathbb{Z}_{12}$ 拆分为两个 $\mathbb{Z}_6$ 子环（偶数一圈、奇数一圈），是模型在 WD 压力下的**自发对称性破缺**。

#### 6.9.5 发现 3：内层都走 stride=4 = gcd(12,8)

不管外层选 stride=1 还是 stride=2，内层最终都收敛到 stride=4。

$4 = \gcd(12, 8)$——外层 12 步一圈，内层 8 步一圈，每 4 步两层相位对齐一次。这是用同一组权重同时编码两层信息的数学最短路径。模型自主发现了这一数论结构。

#### 6.9.6 发现 4：WD 非单调性——超长训练是毒药

wd=2.0 从 1M 步延长到 5M 步：

| 训练步数 | train_acc | test_acc | 状态 |
|----------|-----------|----------|------|
| 1M | 100% | 100% | 甜蜜点 |
| 3.56M | — | 3.4% | 崩溃 |
| 5M | 78.2% | 73.4% | 永久退化 |

**Weight Decay 先逼出结构（Grokking），再摧毁结构（过度压缩）。** 存在最优训练长度，过了就是毒药。

#### 6.9.7 WD 相图

| WD | test_acc (1M步) | 行为 |
|----|-----------------|------|
| 1.0 | 89.6% | 未 Grok，大部分硬背 |
| 1.5 | 100% | Grok，外层 s1→350K崩→内层 s4 接管 |
| 2.0 | 100% | Grok，外层 s2→140K崩→内层 s4 接管 |
| 5.0 | 77.7% | 过度压缩，连硬背都做不到 |

WD 越强，外层死得越快（350K → 140K），但最终结局相同——只有内层 stride=4 存活。

#### 6.9.8 发现 5：扩容实验——简单容量假说不成立

如果拓扑夺舍是因为容量不足，那将模型扩大（4层256维，~3.2M 参数）应该能让两层拓扑共存。

**结果**：train_acc=56.4%，test_acc=51.75%——**连训练集都没拟合，更谈不上 Grok。**

scan_strides 关键数据：

| Step | outer_s1 | inner_s4 | 解读 |
|------|----------|----------|------|
| 360,000 | **1.000** | 0.000 | 外层 s1 瞬间完美（仅存活 1 万步） |
| 380,000 | 0.000 | **1.000** | 内层 s4 瞬间完美（仅存活 1 万步） |
| 400,000+ | 0.000 | 0.1-0.4 | 全部崩溃，漫无目的漂移 |
| 1,000,000 | 0.000 | 0.177 | 最终：无稳定拓扑 |
...
|---|---|---|
| test_acc | 100% | 51.75% |
| 外层锁定持续 | 10-35 万步 | 仅 1 万步（360K闪现） |
| 内层稳定得分 | 0.5-0.9 | 无稳定（漂移） |
| 最终拓扑 | 内层 stride=4 稳定 | 无稳定拓扑 |

**结论**：

不是"房子大了就能两层共存"，而是"房子大了反而找不到墙在哪"。

- 小模型：容量小 → WD=2.0 能有效约束 → 逼出结构 → 夺舍
- 大模型：容量大 → WD=2.0 的压力相对不够 → 参数空间太大，优化地形过于平坦 → 任何拓扑都站不住

大模型在 360K 和 380K 步瞬间触碰到了完美拓扑（outer_s1=1.0 和 inner_s4=1.0），说明目标结构在解空间中**存在**，但优化器无法找到稳定路径。

**推测**：容量与正则化压力需要匹配——大模型可能需要更大的 WD（如 5.0-10.0）才能逼出结构。这与 Epiplexity 可学性实验的结论一致：更大 ≠ 更好，除非同时加压。
...
嵌套 Grokking 实验揭示了一种新现象：当模型容量不足以同时维持两层拓扑时，不是两层共存，而是发生**拓扑夺舍**——后学到的结构取代先学到的结构，全程输出不变。这说明：
- 表示空间的结构可以在输出完全不变的情况下发生彻底重组
- **test_acc=100% 不代表模型的内部理解是稳定的**——同样的准确率下面，可能是完全不同的拓扑组织
- 流形发现不是单向的"从坏到好"，而是多个拓扑候选解之间的动态竞争

**6. 容量-正则化匹配原则**

扩容实验的失败（4层256维，test_acc=51.75%）推翻了"更大就更好"的朴素直觉。大模型在 360K 步瞬间触碰完美拓扑又立即崩溃，说明目标解**存在**于参数空间中，但优化地形过于平坦，优化器找不到稳定路径。这提示一个**匹配原则**：容量和正则化压力必须联合调节——单独加大容量而不增加约束，等于给了更大的房子却没有家具，结构反而不如小房子里逼出来的紧凑。

### 7.4 局限与未来方向

本文的局限：
1. **任务范围有限**：只验证了模加法和模乘法，不清楚是否适用于其他 Grokking 任务（如排列群、多项式求值）
2. **两阶段模型待验证**：模乘法的"全局粘合"是否会在更长训练后发生？（嵌套 Grokking 实验表明：延长训练 + 加大 WD 不会导致全局粘合，而是导致拓扑夺舍，甚至在 5M 步后结构被摧毁）
3. **64 维异常待解释**：目前只有直觉解释，缺乏严格理论
4. **容量-正则化匹配的定量规律**：大模型需要多大的 WD 才能逼出结构？是否存在 $\text{WD} \propto \sqrt{\text{参数量}}$ 的 scaling law？

未来方向：
1. 在更多任务上验证两阶段模型和拓扑夺舍现象
2. 探索是否可以通过操控表示空间拓扑来控制 Grokking
3. 研究 64 维异常的数学机制
4. **容量-正则化联合扫描**：系统搜索模型大小和 WD 的二维相图，寻找"夺舍→共存"的临界曲线
5. **换 gcd=1 的任务**（如 mod 91 = 7×13），验证两层结构是否因天然正交而更容易被同时发现
6. **拓扑闪现的复现性**：大模型在 360K/380K 步的瞬间完美拓扑是偶然还是必经之路？多 seed 能否复现？
...
**嵌套 Grokking 实验**进一步揭示了：
- **拓扑夺舍**：当容量不足以同时编码多层结构时，模型会在 test_acc=100% 的情况下完成内部拓扑的彻底更替
- **WD 非单调性**：正则化先逼出结构再摧毁结构，存在最优训练长度
- **内层走 gcd**：模型自主发现 $\gcd(12,8)=4$ 作为同时编码两层信息的最短路径
- **容量-正则化匹配原则**：单纯加大容量（8倍参数）不但没有实现拓扑共存，反而导致 Grok 失败（test_acc=51.75%）。容量和正则化压力必须联合调节

**一句话概括：高维曲线 → 震荡 → 低维曲面（成功率 ~67%，可能分两步走；容量不足时多层结构会发生拓扑竞争）。**
